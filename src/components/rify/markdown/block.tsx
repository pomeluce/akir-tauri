...
export const BlockView: React.FC<{}> = () => {
  ...
  useEffect(() => {
    ...
    const editor = get();
    if (!editor) return;
    tooltipProvider.current = new BlockProvider({ ctx: editor.ctx, content: div, tippyOptions: { appendTo: div.parentElement || document.body } });

    return () => tooltipProvider.current?.destroy();
  }, [loading]);

  useEffect(() => tooltipProvider.current?.update(view));

  return (
    <div id="rify-milkdown-block">
