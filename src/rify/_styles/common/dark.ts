...
const base = {
  ...
  // primary
  primaryHover: '#5faeff',
  primaryDefault: '#409eff',
  primaryActive: '#388be0',
  primarySuppl: 'rgb(0, 102, 255)',
  ...
  infoDefault: '#909399',
  infoActive: '#7f8187',
  infoSuppl: 'rgb(52, 73, 94)',
};

const baseBackgroundRgb = rgba(base.neutralBase);
const baseInvertBackgroundRgb = rgba(base.neutralInvertBase);
const overlayPrefix = `rgba(${baseInvertBackgroundRgb.slice(0, 3).join(', ')}, `;
const overlay = (alpha: number | string): string => overlayPrefix + String(alpha) + ')';
const neutral = (alpha: number | string): string => {
